...
def test_multiple_sessions_have_isolated_house_rules_and_players():
    ...
    r1 = client.get("/api/house_rules", headers={"X-Session-Key": session_id1})
    assert r1.status_code == 200
    assert r1.json()["table_minimum"] == 10
    assert r1.json()["vig_on_win"] is True

    # GET house rules session 2
    r2 = client.get("/api/house_rules", headers={"X-Session-Key": session_id2})
    ...
    p1 = client.patch("/api/players", headers={"X-Session-Key": session_id1}, json=patch1)
    assert p1.status_code == 200

    # Patch players for session 2: only Fielder
    patch2 = [{"name": "Molly", "active": False}, {"name": "Fielder", "active": True}]
    p2 = client.patch("/api/players", headers={"X-Session-Key": session_id2}, json=patch2)
    assert p2.status_code == 200

    # GET players for session 1
    list1 = client.get("/api/players", headers={"X-Session-Key": session_id1}).json()
    active1 = [p["name"] for p in list1 if p["active"]]
    print("\nSession 1 active players:", active1)
    assert active1 == ["Molly"]

    # GET players for session 2
    list2 = client.get("/api/players", headers={"X-Session-Key": session_id2}).json()
